...
class ChatService:
    ...
    def retrieve_from_web(self, question: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Error retrieving from web: {str(e)}")
            return []
    ...
    def rerank_similarity(self, query: str, documents: List[Dict[str, Any]]) -> List[float]:
        ...
        try:
            ...
            logger.warning(f"Error in rerank_similarity: {str(e)}")
            # 出错时返回原始权重
        ...
        try:
            ...
            for doc in sorted_docs:
                ...
                if total_tokens + doc_tokens > self.max_tokens:
                    logger.info(f"跳过文档: {doc.get('title', '无标题')} ({doc_tokens} tokens)，会超出限制")
                    continue
                
                # 加入文档并累计token数
                filtered_docs.append(doc)
                total_tokens += doc_tokens
                logger.info(f"添加文档: {doc.get('title', '无标题')} ({doc_tokens} tokens), 累计: {total_tokens}/{self.max_tokens}")
            ...
            logger.warning(f"Error in rerank_documents: {str(e)}")
            # 出错时回退到简单排序
            sorted_docs = sorted(documents, key=lambda x: x.get("weight", 0), reverse=True)
