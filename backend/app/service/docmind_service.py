...
class DocMindService:
    ...
    def collect_all_results(self, task_id: str, layout_step_size: int = 10) -> str:
        ...
        while True:
            ...
            logger.info(f"获取到 {len(layouts)} 个布局块 (从 {layout_num} 开始)")
...
def process_document_with_docmind(
    ...
    try:
        ...
        # 7. 插入 Milvus
        logger.info(f"开始插入 Milvus，集合: {index_name}")
        ...
        logger.info(f"文档处理完成: {result['message']}")
