...
from core.database import get_db, resolve_text2sql_url
...
async def text2sql_query(
    ...
    try:
        ...
        # 构建数据库连接字符串：T10 —— 优先只读账号（TEXT2SQL_DATABASE_URL），
        # 未配置时回退主库。解析优先级集中在 core.database.resolve_text2sql_url。
        db_url = resolve_text2sql_url()
