...
| `main` 当前 tip（2026-09-16 第二轮核实，本地＝远端） | `94f1ddb`（**T58 / PR #189 的 merge**，含代码变更；上一核实时点为 `74045f0` = PR #184 台账回填的 merge，其记录的是收口 PR #183 的 merge `e4b4a08`）。⚠️ **字段语义 = 核实时点 tip，不再逐次追平**：纯台账回填 commit **无法**记下自身的 merge SHA（自指）—— 合并后 `main` 必前移，账目又立刻过期，形成无限回归。故此后若 `main` 仅因回填类提交前移，本字段**不再更新**；**审查终点（最后一个含代码变更的 commit）**：第一轮为 `8588718`；**2026-09-16 第二轮（T57–T59）为 `94f1ddb`**（T58 / PR #189 的 merge） |
...
>
> **第三轮（2026-09-16）**：**P-16 已定性并修复**（T61 → issue #191）—— 至此**代码可修类未闭合项清零**，现余 **P-06 / P-08**（环境侧，非代码可修）与 **R-01**（用户决策）。
...
| **P-16** | 大纲审批后的**恢复运行**中大量 SSE 事件未进队列（同一日志 `Queued event` 168 条 vs `No queue available` 246 条，其中 `search_results` 丢 **120** 条）；`base.py:307` 判 `_message_queue` 为 None，`graph.py:434` 接线、`:925` finally 置 None | **已定性 + 已修复（T61 → issue #191；合并核验见执行日志）** | **根因（2026-09-16 定性，证据 `.runlogs/t49_backend8001c.log`）**：`10:25:03.786` 在 `service.py:227 yield` 处被抛入 **`GeneratorExit`**（SSE 客户端断连 → Starlette 关闭响应生成器）→ `_run_simplified` 的 `finally` 把队列置 None，但 `run_agent_with_streaming` 里 `asyncio.create_task` 出来的 agent 任务**无人取消**，于是脱管续跑 **6 分钟**（`10:25:04` → `10:31:45`），期间每条事件都命中 `base.py:314` 的 `No queue available`。**故 `:434` 的接线本身正确 —— 丢失是断连 teardown 缺陷的后果**；客户端全程未调 `/research/cancel`，因此也不会在下一个检查点停手。**修复**：`_run_simplified` 登记在飞任务，`finally` 中**先 cancel 再摘队列**；新增 `test_graph_stream_teardown.py`（不依赖基础设施）+ 变异检验。**未纳入本票**：断连后「后台续跑并持续落检查点」需架构决策（协议 §1 架构分叉），记为独立议题；`GeneratorExit` 展开期的 `ValueError: Token … created in a different Context` 为次要噪声，另行跟踪 |
