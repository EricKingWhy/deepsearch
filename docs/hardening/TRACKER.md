...
| 当前批次 | **阶段 7 第 3 批（已收批）** —— 覆盖 **T10 / T47 / T44 / T45**，fixed point `c469101` → 审查 commit `31491d6` / 修复 commit `187cb90`（PR #167，merge `7cc72f6`）。四票全部 DONE；7-3 findings（5 条：修 1 保 4）已处置，明细见下。**下一步**：**T49 复核已完成**（2026-09-14）—— Docker 前置已解除，但 T08 受 **DocMind 未开通** + **DashScope embedding 免费额度耗尽**阻塞、T35 目标 UI 须先跑完一次深研（同受额度约束）→ **维持 BLOCKED**，待用户侧账号动作后补跑（详见 T49 行、P-13）；**§4 总门禁的最终全量审查**（fixed point `9342913`）须待 T49 解除后执行 |
...
| T49 | needs-infra 验证补跑（T35 实机渲染 + T08 端到端） | #156 | BLOCKED | — | — | — | BLOCKED（**批次 7-3 复核，2026-09-14**：Docker 栈已恢复（7 容器 healthy），上一轮「Docker 未运行」的前置已解除；但两条验收仍**均不可执行**，根因上移到**第三方账号侧** —— ① **T08 端到端**：(a) 文档解析走阿里云 **DocMind**，实测 `submit_job` 返回 `DocMindServiceNotOpen`（`You have not open the docMind service.`）→ 上传的 `华电科工.pdf` 处理状态 `failed`；(b) 向量化走 **DashScope text-embedding-v4**，实测 `HTTP 400 AllocationQuota.FreeTierOnly`（免费额度耗尽且账号处于「仅用免费额度」模式）→ `generate_embedding` 返回 `None`。两条都是**账号 / 计费动作**，AI 无法代劳。② **T35 实机渲染**：浏览器前置已满足（本机 Chrome + Edge 可用），但 echarts 三处（`knowledge-graph` / `process-report` / `visualization`）均为**深研结果详情页的子组件、无独立路由**，必须先跑完一次深研才能到达，而深研同样受第三方额度约束 → 未执行。**处置**：本次已把 T08 推进到「上传 → 解析」边界并**实测拿到外部错误原文**（比上一轮「Docker 未运行」可执行得多）；**未伪造任何通过**。验证用数据（`demo` 知识库 + 失败文档行）已经 API 删除、`knowledge_bases` 计数归零，宿主 8001 后端已停。**待用户在阿里云侧开通 DocMind / 恢复 embedding 额度后补跑**） | §4 残留 | — |
