...
| `main` 当前 tip（2026-09-16 核实，本地＝远端） | `d5a0a9d20701aac9ae8beb71cf79cef7663fe6b8`（**T53 修复 PR #174 的 merge**；其前为 T52 PR #172 = `63c0574`、T51 PR #170 = `8ff3412`。更早：批次 7-3 审查修复 PR #167 merge = `7cc72f6`、T45 PR #166 = `31491d6`、T44 PR #165 = `7bd0fc0`、T47 PR #164 = `3b8ca0f`、T10 PR #163 = `6454b9e`） |
| 当前批次 | **阶段 7 第 3 批（已收批）+ T49 复核衍生修复（T51 / T52 / T53）** —— 批次覆盖 **T10 / T47 / T44 / T45**，fixed point `c469101` → 审查 `31491d6` / 修复 `187cb90`（PR #167）。**T49 复核衍生**：复盘 09-14 日志挖出 **P-14**（已修 T51 / PR #170）、**P-15**（已修 T52 / PR #172）、**P-16**（仅观察）。**2026-09-16 T08 端到端补跑成功**（上传 → Milvus `kb_demo` 27 行 → 本地检索命中 3 条，证据 `.runlogs/t49_r3_verify.out` + `.runlogs/t49_r3_backend_evidence.log`），并**在同一次补跑中挖出 P-17**：后台文档处理的临时文件清理异常会打死整个 uvicorn 进程 → 立项 T53。**下一步**：T53 合并 → T49 的 T35 实机渲染 → **§4 总门禁最终全量审查**（fixed point `9342913`） |
| 当前 fixed point（上一批审查结束 commit） | `187cb90`（阶段 7 第 3 批审查修复 commit）；§4 总门禁审查起点 = `9342913` |
| 当前分支命名 | `T<编号>-<短描述>`（**必须扁平，禁止 `/`**，见协议 §9.1） |
| 合并目标 | 本地 `main` 分支（merge commit，不用 squash） |
| 总 ticket 数 | 54（T01–T41 + 阶段 7 的 T42–T50 + 追加的 T51–T54） |
| 已完成 | 53（阶段 1–6 的 40 + 阶段 7 的 T42 / T43 / T46 / T48 / T50 / T10 / T47 / T44 / T45 + T51 + T52 + T53） |
...
| T49 | needs-infra 验证补跑（T35 实机渲染 + T08 端到端） | #156 | BLOCKED（仅剩 T35） | — | — | — | BLOCKED（**2026-09-16 三次推进**：① **T08 半边已闭合** —— 端到端实测「上传 → DocMind 27 切片 → 1024 维向量 → 写入 Milvus `kb_demo`」成功，独立复核 `row_count=54`、`retrieve_from_knowledge_base("demo")` 命中 3 条且 url 全为 `local://kb/demo/…` （证据 `.runlogs/t49_r3_verify.out` + `.runlogs/t49_r3_backend_evidence.log`）；② **「无可用对话模型」这一核心阻塞已解除** —— 探针实测 DashScope 仍 403（Free quota exhausted），但**硅基流动那套凭据（现用于 embedding）同时支持对话**：`Qwen/Qwen2.5-7B-Instruct` → HTTP 200；同时确认 `OPENROUTER_API_KEY` 就是**未填的占位符**（值前缀 `your-o…`，实测 401 Missing Authentication header）；③ 故 **T35 实机渲染已具备执行前提**（本机 `.env` 把对话模型指向硅基流动即可，属本地配置、不入库）。**顺带**：该轮补跑暴露并修复了 **P-17**（后台清理异常打死服务 → T53）。（下略，见 2026-09-14 记录）**2026-09-14 晚二次复核，前置大幅推进**：① **embedding 已切换硅基流动**并随 PR #168 落地供应商配置化 —— 实测 `generate_embedding` 经硅基流动 `BAAI/bge-m3` 返回 **1024 维**（与 `milvus_service.vector_dim` 一致），且直接探针证实其**不接受 `dimensions` 参数**（400，code=20015），故该参数改为「配置了才传」（详见 PR #168）；② **T08 检索半程已用真实基础设施验证通过**：手工种入 `kb_demo`（3 chunks，走应用自身 `generate_embedding` + `milvus_service.insert_documents`）后，`retrieve_from_knowledge_base('demo')` 命中、url 前缀 `local://kb/demo/`、V2 `DeepScout._execute_local_search` 返回统一形状（`is_local` / `kb_name`）—— **T08 修复的集合名口径（`kb_<name>` 而非 `"knowledge_base"`）在真实 Milvus 下闭环**；③ **仍不可执行的部分与根因**：(a) T08 ingestion 半程 —— DocMind **服务已开通**（用户 2026-09-14 操作，免费额度 3000 页），但探针实测 `NoPermission — You are not authorized to perform this operation.`，即该 AccessKey 所属 RAM 身份**未被授予 DocMind 权限策略**（需在 RAM 控制台附加 `AliyunDocmindFullAccess`，账号侧动作）；(b) **T35 与 T08 的「发起 v2 研究」都需要 LLM** —— 探针实测 DashScope chat（`qwen3.6-plus`）**403 Free quota exhausted**（与 embedding 同一账号限制），而 `OPENROUTER_API_KEY` 长度仅 23 且非 `sk-or-` 前缀（**疑似无效 / 占位**，实测 401）。**处置**：验证残留已全部清理（`kb_demo` 已 drop、Milvus collections 归零、`demo` 知识库 DELETE 204、`knowledge_bases` / `documents` 计数归零、8001 宿主后端已停）。**待用户**：① RAM 授予 DocMind 权限；② 恢复任一可用 LLM（DashScope 充值 / 关闭「仅用免费额度」，或提供有效 OpenRouter key）后补跑完整端到端） | §4 残留 | — |
...
| T52 | 前端「本地知识库」模式从不传 kb_name，导致静默零结果（P-15） | #171 | DONE | `T52-deepsearch-kb-name` | `69d7daa` + `99b9aeb` + `96fae36` | #172 | PASS（`npm run test` → **37 passed / 6 files**（基线 33 → +4）；`npx tsc --noEmit` 无输出；`npx eslint .` 无输出；**变异检验**：`kb_name` 取值恒 `undefined` → 1 failed、去掉 `local` 门控 → 1 failed、还原后 18 passed。CI backend pass 1m4s / frontend pass 43s；`gh issue view 171` → **CLOSED**（`Closes #171` 生效）） | 追加 | FIXED@63c0574 |
...
| T54 | 把 T35 的浏览器渲染风险条款固化为可执行 e2e（ECharts 真机渲染） | #175 | DOING | `T54-echarts-render-e2e` | （本分支） | — | DOING（新增 `frontend/e2e/echarts-lazy-render.spec.ts`，两条用例覆盖 `visualization.tsx` / `knowledge-graph.tsx` 两个挂载点：按需加载 + canvas 真的有不透明像素 + 桩自足（无请求漏到真实后端）+ 无未捕获异常。实测 **2 passed**；变异检验 M-A（换回静态 import）/ M-B（空 option）各 1 failed、还原后 2 passed 且 sha256 一致。**复核发现 T35 当年「本机无浏览器自动化环境」的前提不成立**：`PLAYWRIGHT_BROWSERS_PATH` 指向 `D:\DevTools\Hermes\ms-playwright`，其下 chromium-1228 / chromium-1243 早已就位） | 追加 | PENDING |
