...
| 当前 fixed point（上一批审查结束 commit） | `187cb90`（阶段 7 第 3 批审查修复 commit）。**§4 总门禁（第三轮）fixed point = 计划起始基线 `9342913`，审查终点 = `d128d83`（最后一个含代码变更的 commit = `43f6cd1`）；本轮修复 = T63 的 4 个 commit（台账记录于本记录 commit），收口 commit = PR #197 的 merge（合并后回填）** |
...
| §4-3 | 全量（第三轮门禁） | `9342913` | `d128d83` | 17 | T63 的 4 个 commit | FIXED |
...
## §4 总门禁（第三轮）findings 明细（`9342913` → `d128d83`，修复 = T63 的 4 个 commit）
