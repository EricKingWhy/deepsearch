...
| `main` 当前 tip（2026-09-16 核实，本地＝远端） | `8ff3412efb67c296df0ec761ec319d00ff416226`（**T51 修复 PR #170 的 merge**；其上为 T52 的修复分支 `T52-deepsearch-kb-name`，合并后回填。此前：批次 7-3 审查修复 PR #167 merge = `7cc72f6`、T45 PR #166 = `31491d6`、T44 PR #165 = `7bd0fc0`、T47 PR #164 = `3b8ca0f`、T10 PR #163 = `6454b9e`） |
| 当前批次 | **阶段 7 第 3 批（已收批）+ T49 三次复核（2026-09-16）** —— 批次覆盖 **T10 / T47 / T44 / T45**，fixed point `c469101` → 审查 `31491d6` / 修复 `187cb90`（PR #167）。**2026-09-16 复盘 09-14 那次 T49 补跑的后端日志**：09-14 记的「T08 端到端通过」**缺少落盘证据**（`local://kb/demo` 全 `.runlogs/` 命中 0、脚本 stdout 未保存）→ 降级为「曾运行、未证实送达」；并从中挖出 **P-14**（`scout.py` 的 `source_url` 数组值使检索阶段静默崩溃 → **已立项 T51 并修复**）、**P-15**（前端从不传 `kb_name`，「本地知识库」必然静默零结果 → 用户裁定**前端补知识库选择器**，立项 T52）、**P-16**（恢复运行中 SSE 事件大范围未入队，仅观察、未定根因）。**下一步**：T51 / T52 合并 → 补跑 T49（需先起 Docker）→ **§4 总门禁最终全量审查**（fixed point `9342913`） |
...
| 总 ticket 数 | 51（T01–T41 + 阶段 7 的 T42–T50 + 追加的 T51） |
| 已完成 | 50（阶段 1–6 的 40 + 阶段 7 的 T42 / T43 / T46 / T48 / T50 / T10 / T47 / T44 / T45 + T51）__+ T52（本分支已实现并验收，待 PR 合并）__ |
...
| T52 | 前端「本地知识库」模式从不传 kb_name，导致静默零结果（P-15） | #171 | DOING | `T52-deepsearch-kb-name` | `69d7daa` + `99b9aeb` | — | PASS（`npm run test` → **37 passed / 6 files**（基线 33 → +4）；`npx tsc --noEmit` 无输出；`npx eslint .` 无输出；**变异检验**：`kb_name` 取值恒 `undefined` → 1 failed、去掉 `local` 门控 → 1 failed、还原后 18 passed） | 追加 | PENDING |
...
| **P-14** | `deep_research_v2/agents/scout.py` 的 `sources_count` 在 `source_url` 为大模型返回的**数组**时抛 `TypeError: unhashable type: 'list'`；异常从 `process()` 逸出后被 `graph.py` 记为 `Task exception was never retrieved` → 检索阶段**静默死亡**（不发 `search_results`，UI 恒 `charts=0`） | ✅ **已修复（T51，分支 `T51-scout-source-url-normalize`，待合）** | 2026-09-14 补跑 T49 的浏览器会话实测命中（主题为「欧盟人工智能法案 / 终身学习补贴 / OECD」）；离机复现 `.runlogs/p14_repro.py`（不依赖 Docker / LLM / 网络）。单一边界归一（3 处 fact 写入）+ 聚合双保险（2 处） |
| **P-15** | 前端**从不传 `kb_name`**（`grep -rn "kb_name" frontend/src` 零命中；`chat/index.tsx:320` 只发 `search_modes`），而后端 `Scout._execute_local_search` 见 `kb_name` 为空即跳过 → 勾选「本地知识库」**必然零结果且无任何报错**（日志中该告警 18 次，全部来自浏览器会话） | ✅ **已修复（T52，分支 `T52-deepsearch-kb-name`，待合）** | 这是「本地知识库」模式在 UI 上**完全不可用**的功能缺陷；T49 的 T35 实机渲染若走本地模式亦被其挡住 |
