...
| `main` 当前 tip（2026-09-16 第三轮核实，本地＝远端） | `8cad1f0`（**T61 / PR #192 的 merge**，含代码变更；上一核实时点 `94f1ddb` = T58 / PR #189 的 merge）；上一核实时点为 `74045f0` = PR #184 台账回填的 merge，其记录的是收口 PR #183 的 merge `e4b4a08`）。⚠️ **字段语义 = 核实时点 tip，不再逐次追平**：纯台账回填 commit **无法**记下自身的 merge SHA（自指）—— 合并后 `main` 必前移，账目又立刻过期，形成无限回归。故此后若 `main` 仅因回填类提交前移，本字段**不再更新**；**审查终点（最后一个含代码变更的 commit）**：第一轮为 `8588718`；**2026-09-16 第三轮（T61）为 `8cad1f0`**（T61 / PR #192 的 merge） |
...
>
> **第三轮（2026-09-16）**：**P-16 已定性、修复并关闭**（T61 → PR #192 / merge `8cad1f0`；issue #191 **CLOSED**）—— 至此**本轮收口项清零**。现余 **P-06 / P-08**（环境侧，非代码可修）、**R-01**（用户决策），以及同族新票 **T62**（issue #193，PR #195）—— 它同样已修复：`GeneratorExit` 展开期 contextvar token 跨 Context 重置报错被守卫消除。至此**「断连 teardown 族」整体收口**。
