...
| 当前 fixed point（上一批审查结束 commit） | `187cb90`（阶段 7 第 3 批审查修复 commit）。**§4 总门禁（第三轮）fixed point = 计划起始基线 `9342913`，审查终点 = `d128d83`（最后一个含代码变更的 commit = `43f6cd1`）；本轮修复 = T63 的 7 个 commit（台账记录于 `eb67dad`），收口 commit = PR #197 的 merge（合并后回填）** |
...
| 7 | 规格·中 | `main` tip 行的「**审查终点**（最后一个含代码变更的 commit）」仍停在 `99cc027`，而 PR #196 本身**含代码变更**（`context.py` + 测试），豁免条款（仅回填）不适用 | **已修**：终点改为含代码变更的当前 tip（`43f6cd1`；本次合并后回填收口 merge） |
