...
| `main` 当前 tip（2026-09-16 核实，本地＝远端） | `c83f26c37a30a92ddc06d0dff399a0dbd93afb48`（T49 二次复核台账；其上为**追加票 T51 的修复分支** `T51-scout-source-url-normalize` —— PR 合并后回填 merge SHA。此前：批次 7-3 审查修复 PR #167 merge = `7cc72f6`、T45 PR #166 = `31491d6`、T44 PR #165 = `7bd0fc0`、T47 PR #164 = `3b8ca0f`、T10 PR #163 = `6454b9e`） |
...
| 已完成 | 49（阶段 1–6 的 40 + 阶段 7 的 T42 / T43 / T46 / T48 / T50 / T10 / T47 / T44 / T45）__+ T51（本分支已实现并验收，待 PR 合并）__ |
...
| **P-15** | 前端**从不传 `kb_name`**（`grep -rn "kb_name" frontend/src` 零命中；`chat/index.tsx:320` 只发 `search_modes`），而后端 `Scout._execute_local_search` 见 `kb_name` 为空即跳过 → 勾选「本地知识库」**必然零结果且无任何报错**（日志中该告警 18 次，全部来自浏览器会话） | **已立项 T52**（用户裁定：前端补知识库选择器） | 这是「本地知识库」模式在 UI 上**完全不可用**的功能缺陷；T49 的 T35 实机渲染若走本地模式亦被其挡住 |
