...
| 已完成 | 55（阶段 1–6 的 40 + 阶段 7 的 T42 / T43 / T46 / T48 / T50 / T10 / T47 / T44 / T45 + T51 + T52 + T53 + T54 + T55） |
...
| T56 | document_router 的临时文件清理仍是裸 os.remove（P-17 同族残留） | #179 | DOING | `T56-document-cleanup-guard` | （本分支） | — | DOING（把 `core/upload_security._remove_quietly` 提升为公共 `remove_quietly(path, *, logger=None)`，`document_router` 两处清理改走守卫；新增 `backend/tests/router/test_document_cleanup_guard.py` **12 例**（守卫层 / 端点层 / 源码层，不依赖基础设施）；**变异检验**：M1 改回裸 os.remove → 3 failed、M2 守卫去掉 except OSError → 3 failed、M3 守卫改 no-op → 4 failed，还原后 12 passed 且两文件 sha256 一致。全量 `pytest tests -q` → **401 passed / 17 deselected**） | 追加 | PENDING |
