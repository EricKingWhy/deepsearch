...
| `main` 当前 tip（2026-09-16 核实，本地＝远端） | `74045f0`（**台账回填 PR #184 的 merge**；其记录的内容即收口 PR #183 的 merge `e4b4a08`，后者含 3 个 commit：`9911204`（test 假红修复）/ `d8f1821`（台账回填）/ `0eb74b5`（表格修复））。⚠️ **字段语义 = 核实时点 tip，不再逐次追平**：纯台账回填 commit **无法**记下自身的 merge SHA（自指）—— 合并后 `main` 必前移，账目又立刻过期，形成无限回归。故此后若 `main` 仅因回填类提交前移，本字段**不再更新**；**审查终点（最后一个含代码变更的 commit）恒为 `8588718`** |
