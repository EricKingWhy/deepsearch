...
> **Two research routes coexist by design — do NOT delete either retained implementation:**
...
> is an intentionally retained alternative (**PRD NG-3**). V1 is unreachable on the daily default path — that does
> not make it dead code. The LangGraph runtime inside `deep_research_v2/graph.py` is likewise a retained parallel
> implementation (**PRD NG-2**; the hand-written async state machine `_run_simplified` is what actually runs today).
